...
public class TestcaseFile extends TestcaseElementWriterBase implements TestcaseElementWriter {

    @Override
    public int writeExcel() {
        String elementName = getElementLabel(ExcelKey.OBJECT_FILE);
        writeTestcase(elementName);
        return activeRow;
    }

    private void writeTestcase(String elementName) {
        writeInputValue(ActionType.INPUT_FILE, elementName);
        writeClearValue(ActionType.CLEAR_FILE, elementName);
        writeAssertValue(ActionType.ASSERT_FILE, elementName);
